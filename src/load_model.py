...
def main(argv=None):
    ...
    for t, (_, ncla) in enumerate(taskcla):
        ...
        for u in range(t + 1):
            ...
            #torch.onnx.export(appr, dummy_data, "appr.onnx")
            #exit()
        ...
        # Save
        #x = torch.empty(1, 3, 32, 32, dtype = torch.float32).to(device)
        #torch.onnx.export(appr.model.to(device),               
                  #x,                         
                  #"appr.onnx",   
                  #export_params=True,        
                  #opset_version=10,         
        ...
        #Load
        onnx_model = onnx.load("appr.onnx")
